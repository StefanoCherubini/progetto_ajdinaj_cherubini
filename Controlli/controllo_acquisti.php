...
// Recupera la password utente dal DB
$query_utente = "SELECT password FROM utenti WHERE id_utenti = $id_utenti";
...
$row = mysqli_fetch_assoc($result_utente);
$password_db = $row['password'];
$password_post = $_POST['password'];

// Variabile per l'errore della password
$errore_password = null;

// Confronto diretto tra la password passata via POST e quella nel database
if ($password_post !== $password_db) {
    $errore_password = "⚠️ Password errata";
}

// Se la password è corretta, continua con la logica degli acquisti
if ($errore_password === null) {
    $posti = [];
    ...
    foreach ($posti as $posto) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // Inserisce il posto come non disponibile
...
<body class="pag2">
    ...
    <div class="position-relative container text-white">
        ...
        <div class="position-absolute start-50 translate-middle-x">
            <?php if (!empty($inseriti) && $errore_password === null): ?>
            ...
            <?php if ($errore_password !== null): ?>
                <h2><?= $errore_password ?></h2>
                <br>
                <ul>
                    <h4>Sembra che la password inserita non sia corretta.</h4>
                    <h6>Per favore, riprova inserendo la password corretta.</h6>
                </ul>
            <?php endif; ?>
